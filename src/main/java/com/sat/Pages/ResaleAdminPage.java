...
public class ResaleAdminPage {
	...
	@FindBy(xpath="//div[@data-control-name='icnBackMAS_9']")
	//(//*[name() = 'svg'][contains(@data-appmagic-icon-name, 'Basel_ArrowLeft')])[1]
...
//				robot.keyRelease(KeyEvent.VK_CONTROL);
	...
	public void selectedStore(String brand, String country, String store) throws InterruptedException {
		...
		while (!flag1 && (System.currentTimeMillis() - startTime) < 60000) {
			...
			System.out.println("Size of the pencil icon"+penIcon.size());
			for (int i = 0; i < penIcon.size(); i++) {
				if (isClickable(penIcon.get(i))) {
					Wait.elementToBeClickable(driver, penIcon.get(i), 5);
		...
		Wait.elementToBeClickable(driver, brandList, 5);
		brandList.click();
		...
		Wait.elementToBeClickable(driver, brandSelection, 5);
		brandSelection.click();
		Wait.elementToBeClickable(driver, countryList, 5);
		countryList.click();
		//selecting another country as dataverse issue
		WebElement countrySelection1 = driver.findElement(By.xpath("//div[contains(@id,'appmagic-dropdownFlyoutdropdown_siena_') and text()='Sweden']"));
		Wait.waitForPresenceOfElementLocated(driver, countrySelection1, 5);
		countrySelection1.click();
		//Now selecting the correct country
		Wait.elementToBeClickable(driver, countryList, 5);
		countryList.click();
		...
		Thread.sleep(5000);
		Wait.elementToBeClickable(driver, storeList, 10);
		storeList.click();
		...
		while (!flag) {
			...
			Wait.elementToBeClickable(driver, okBtn, 5);
			...
			System.out.println("Displayed Store value"+viewselectedStore.getText());
			System.out.println("Displayed Store"+ storeSelection.getText());
...
//			if (yellowpopup.size() == 0) {
				System.out.println("No popups so clicking on back button ");
				//Wait.waitUntilElementVisible(driver,NewHomeBackButton);
			//Wait.elementToBeClickable(driver,NewHomeBackButton , 5);
				NewHomeBackButton.click();
